package fr.ign.simplu3d.iauidf.predicate;
...
public class PredicateIAUIDF<O extends AbstractSimpleBuilding, C extends AbstractGraphConfiguration<O, C, M>, M extends AbstractBirthDeathModification<O, C, M>>
	...
	public boolean check(C c, M m) {
		...
		if (MultipleBuildingsCuboid.ALLOW_INTERSECTING_CUBOID) {
			if (!testWidthAndElongation(c, m, 7.5, distanceInterBati, ELONGATION_MAX)) {
				return false;
			}
		} else {
			if (!checkDistanceInterBuildings(c, m, distanceInterBati)) {
				return false;
			}

			if (!checkElongation(m.getBirth().get(0).toGeometry(), ELONGATION_MAX)) {
				return false;
			}
		...
		if (birth != null) {
			...
			if (birth instanceof ParallelCuboid || birth instanceof ParallelTrapezoid2
					...
					|| birth.getClass().equals(ParallelCuboidRoofed.class) || birth instanceof LBuildingWithRoof) {
			...
			} else if ((birth instanceof ParallelCuboid2) || (birth instanceof ParallelCuboidRoofed2)) {
	...
	public boolean checkBandRegulation(Regulation r, O cuboid) {
		 ...
		 * 
		 * // ART_6 Distance minimale des constructions par rapport à la voirie //
		 * imposée en mètre 88= non renseignable, 99= non réglementé // On vérifie la
		 * contrainte de recul par rapport au front de parcelle // (voirie) // Existe t
		 * il ? int r_art6 = r.getArt_6(); if ( this.jtsCurveLimiteFrontParcel != null
		 * && r_art6 != 88 && r_art6 != 99) { // On vérifie la distance if
		 ...
		 * (jtsCurveLimiteLatParcel != null && r_art72 != 88 && r_art72 != 99) { // On
		 * vérifie la distance if
		 ...
		 * 
		 * int r_art73 = r.getArt_73(); if (jtsCurveLimiteFondParcel != null && r_art73
		 * != 88 && r_art73 != 99) { // On vérifie la distance (on récupère le foot if
		 * (this.jtsCurveLimiteFondParcel.distance(cuboid.toGeometry()) < r_art73) { //
		 * elle n'est pas respectée, on retourne faux return false;
		 * 
		 * }
		 * 
	...
	private boolean testWidthAndElongation(C c, M m, double widthBuffer, double distanceInterBati,
		...
		for (List<AbstractSimpleBuilding> lAb : lGroupes) {
			...
			if (!checkWidthAndElongation(lAb, widthBuffer, elongationMax)) {
				return false;
			}
	...
	private boolean checkElongation(Geometry geom, double elongationMax) {

		OrientedBoundingBox oBB = new OrientedBoundingBox(JTS.fromJTS(geom));

		double largeur = oBB.getWidth();
		double longueur = oBB.getLength();
		double elongation = longueur / largeur;
