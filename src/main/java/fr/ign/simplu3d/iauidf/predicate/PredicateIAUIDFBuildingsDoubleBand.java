...
public class PredicateIAUIDFBuildingsDoubleBand<O extends AbstractSimpleBuilding, C extends AbstractGraphConfiguration<O, C, M>, M extends AbstractBirthDeathModification<O, C, M>>
		extends PredicateIAUIDF<O, C, M> {
	...
	public PredicateIAUIDFBuildingsDoubleBand(BasicPropertyUnit bPU, Regulation r1, Regulation r2) throws Exception {
		super(bPU, r1, r2);
	...
	public boolean checkParcelRegulation(Regulation r, C c, M m) {
		 ...
		 * 
		 * if (reg14 != 0.0 & reg14 != 99 & reg14 != 88) { if (shonBuilt /
		 * areaBPU > reg14) { return false; } }
		 */
	...
	private boolean checkBandRegulationSpecArt71(Regulation r, O cuboid) {
		...
		if (! this.currentBPU.getGeom().contains(cuboid.getFootprint())) {
				return false;
		}
	...
	private boolean checkWidth(List<AbstractSimpleBuilding> lO, double widthBuffer) {
		...
		if (union instanceof Polygon) {
			union = gf.createPolygon(((Polygon) union).getExteriorRing().getCoordinates()).buffer(5).buffer(-5);
		...
		if (negativeBuffer.isEmpty() || negativeBuffer.getArea() < 0.001) {
			...
			if (c % 10000 == 0 || multi) {
				...
				System.out.println("good width " + (negativeBuffer.isEmpty() ? "empty" : negativeBuffer));
