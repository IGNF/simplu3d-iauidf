package fr.ign.simplu3d.iauidf.openmole;
 ...
 * l'expérimentation avec l'IAUIDF
 * 
 * @author ilokhat
 * @author mbrasebin
 */
public class EPFIFTask {
	...
	// parcels with no rules
	private static List<String> idparSimulIsZero = new ArrayList<>();
	...
	public static String run(File folder, String dirName, File folderOut, File parameterFile, long seed)
		...
		for (BasicPropertyUnit bPU : env.getBpU()) {
			...
			if (lR != null && !lR.isEmpty()) {
				...
				// @DESACTIVATED
				/*
				 * double r_art5 = lR.get(0).getArt_5(); if (r_art5 != 99 || r_art5 != 88) { if
				 * (bPU.getPol2D().area() < r_art5) { result += imu + " ; " + id + " ; " +
		...
		// || "
		// + "SDP : " + sd;
		// if (idparSimulIsZero.size() > 0) {
		// for (String id : idparSimulIsZero)
		// result += imu + " ; " + id + " ; " + (CODE_SIMUL_ZERO) + " ; " +
		// (CODE_SIMUL_ZERO) + "\n";
		//
		// }
	...
	private static Map<String, List<Regulation>> loadRules(File parcelle, int imu) {
		...
		for (IFeature feat : featC) {
			...
			if (bande != 0) {
				...
				double art_10_2 = Double
						.parseDouble(feat.getAttribute(ParcelAttributeTransfert.att_art_10_2).toString()); // ATTENTION
				// A
				// CHANGER
				double art_10_m_2 = Double
						.parseDouble(feat.getAttribute(ParcelAttributeTransfert.att_art_10_m_2).toString());
